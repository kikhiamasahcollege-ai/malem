...
export const createLocalAuthService = async ({ dataFile, discoverProvider = discoverPlaces }) => {
  ...
  const tripPayload = (tripId, userId) => {
    ...
    return {
      metadata: { ...metadata },
      trip: JSON.parse(JSON.stringify(record.document)),
    ...
    try {
      ...
      if (parts[0] === 'trips' && parts[1]) {
        ...
        if (parts[2] === 'document' && parts.length === 3 && method === 'PATCH') {
          ...
          const after = applyTripOperations(before, body.operations, { actor: auth.user.id, cryptoApi });
          const now = Date.now();
          const mutation = {
            id: cryptoApi.randomUUID(),
            tripId,
          ...
          database.trips[tripId] = {
            ...database.trips[tripId],
            title: after.summary || after.destination,
