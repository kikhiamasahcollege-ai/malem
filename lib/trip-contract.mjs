...
const placeAfter = (list, value, afterId) => {
  if (afterId === '') list.unshift(value);
  else if (afterId == null) list.push(value);
  else {
...
export const applyTripOperations = (source, operations, {
  ...
  for (const operation of operations) {
    ...
    } else if (type === 'packing.section.move') {
      const sections = document.bundle.packing.lists;
      const index = sections.findIndex((candidate) => candidate.id === operation.sectionId);
      const section = requireEntity(index >= 0 ? sections[index] : null, 'Packing section');
      sections.splice(index, 1);
      placeAfter(sections, section, operation.afterId);
